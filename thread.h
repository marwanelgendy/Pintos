...
struct child_processes
{
  ...
  bool loaded;  // to indicate if child has returned from load function or not.
...
struct thread
  {
    ...
    struct list child_processes;               /* All child_processes that haven't been waited for.
                                        (After a successful wait, child is removed from list) */
    int exit_status;
    struct lock wait_child;
    struct condition parent_waiting_condition;

    int fd_last;         /* value used to generate a new fd integer - initialized to 2 */

    struct list files;                  /* List of opened files */
    struct file* executing_file;        /* Pointer to the process file */

    struct semaphore parent_child_sync;         /* Used to synchronize between child and parent threads */
...
struct open_file *get_open_file (struct thread *t, int fd);
